...
\cfoot{%
  \cistercian[height=0.5cm]{\value{page}}
}
...
\date{Version v0.6 \textendash{} 2025/10/08}
...
\end{tcblisting}
Thanks to using the \saminline|BusyPanda| package, the stroke width will automatically adapt to blend in with the surrounding font:
\begin{tcblisting}{title={Stroke (\textbackslash BusyPanda\{0.125\}*height)},environment lower args={scalebox}{5}}
\cistercian{7}
\fontfamily{ugq}\cistercian{7}
...
Besides using these options as optional arguments, they can also be set globally via the macro \saminline|\cistercianset|:
